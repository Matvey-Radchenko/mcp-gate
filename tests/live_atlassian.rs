...
#[tokio::test]
#[ignore = "Needs explicit VPN/live-read authorization, LIVE_ATLASSIAN_CONFIG and CODEX_BINARY; starts no model"]
async fn two_native_clients_read_live_api() {
    let path = std::path::PathBuf::from(
        std::env::var_os("LIVE_ATLASSIAN_CONFIG").expect("Set pilot config"),
    );
    ...
    let (av, bv) = tokio::join!(
        a.call(
            "jira_search",
            json!({"jql":"assignee = currentUser() ORDER BY updated DESC","limit":1})
        ),
        b.call("confluence_search", json!({"query":"type=page","limit":1}))
    );
    successful_read(&av);
    successful_read(&bv);
    ...
    assert_eq!(health(&config).await["workers"], 1);
    eprintln!(
        "Live Atlassian: 98 tools, two native clients, read-only calls, one shared worker, independent close passed. No API payloads logged."
    );
}
